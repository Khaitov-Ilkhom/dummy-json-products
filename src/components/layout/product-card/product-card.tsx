        ...
        <CardContent className="md:pb-4 pb-2 md:px-4 px-2">
          ...
          <div className="text-md flex flex-col md:flex-row justify-between">
            <p className="flex gap-2 items-end">
              <span className="text-md font-semibold text-green-500">${(product.price - (product.price * product.discountPercentage) / 100).toFixed(1)}</span>
              <span className="line-through text-sm text-red-500"> ${product.price}</span>
            </p>

            <RenderRating rate={product.rating}/>
          </div>
          <p className="w-full flex flex-col md:flex-row justify-between"><span className="hidden md:flex">Status: {product.availabilityStatus}</span> <span>Stock: {product.stock}</span></p>
        </CardContent>
      </Card>
  )
}
